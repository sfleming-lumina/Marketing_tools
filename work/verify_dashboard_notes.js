const { installFakeDom, loadDashboardScript } = require("./dom_fake");

installFakeDom(["feedbackFilter", "feedbackTypeFilter", "feedbackTable", "metrics", "objectTable"]);

const campaignRows = [
  { campaign: "Efficient Search", campaignRollup: "Paid Search", month: "2026-06-01", leads: 100, wins: 20, spend: 20000, revenue: 200000 }
];
global.fetch = url => Promise.resolve({
  ok: true,
  json: () => Promise.resolve(String(url).includes("campaign-performance") ? campaignRows : [])
});

const script = loadDashboardScript();
const run = new Function(`${script}
return (async () => {
  await new Promise(resolve => setImmediate(resolve));
  allNotes = [
    { note_id: "1", created_at: "2026-07-15T10:00:00Z", author_name: "Jane", view: "campaigns", element_key: "campaign:Paid Search:Efficient Search", element_label: "Efficient Search", note_text: "Great campaign card", context: {} },
    { note_id: "2", created_at: "2026-07-15T11:00:00Z", author_name: "Sam", view: "overview", element_key: "metric:projected-revenue", element_label: "Projected revenue", target_type: "metric", feedback_type: "tweak", note_text: "Confusing metric", context: {} }
  ];
  state.view = "campaigns";
  await renderCampaignPlanner();
  state.view = "overview";
  renderMetrics();
  renderObjects();
  return {
    cards: document.getElementById("campaignCards").innerHTML,
    recommendations: document.getElementById("campaignRecommendations").innerHTML,
    metrics: document.getElementById("metrics").innerHTML,
    objects: document.getElementById("objectTable").innerHTML
  };
})();`);

run().then(output => {
  function assert(condition, message) {
    if (!condition) {
      console.error(message);
      process.exit(1);
    }
  }

  assert(output.cards.includes('data-note-key="campaign:Paid Search:Efficient Search"'), "Campaign card is missing its note chip key.");
  assert(output.cards.includes('<span class="note-chip-count">1</span>'), "Campaign card note badge should show a count of 1.");
  assert(output.cards.includes('data-note-target-type="tile"'), "Campaign card note chip should identify tile target type.");
  assert(output.recommendations.includes('data-note-key="campaign:Paid Search:Efficient Search"'), "Campaign recommendation should reuse the campaign entity key.");
  assert(output.metrics.includes('data-note-key="metric:projected-revenue"'), "Projected revenue metric is missing its note chip key.");
  assert(output.metrics.includes('data-note-target-type="metric"'), "Metric note chip should identify metric target type.");
  assert(output.objects.includes('data-note-key="object:analytics_rpt.rpt_marketing_lead_cohort_performance"'), "Object row is missing its note chip key.");
  assert(output.objects.includes('data-note-target-type="object"'), "BQ object note chip should identify object target type.");

  console.log("Dashboard notes key wiring verified OK.");
}).catch(error => {
  console.error(error);
  process.exit(1);
});
